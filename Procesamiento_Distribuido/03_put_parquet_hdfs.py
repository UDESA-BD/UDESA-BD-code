...
# Reemplazando ',' por '.' en duracion_recorrido para que Pandas pueda guardarlo como float.
df['duracion_recorrido'] = df['duracion_recorrido'].str.replace(',', '')
df.astype({'duracion_recorrido': 'float'})

# Extraemos el mes para el particionado
df["mes"] = [f.month for f in pd.to_datetime(df['fecha_origen_recorrido'])]

# Cargamos el DataFrame en memoria con PyArrow
